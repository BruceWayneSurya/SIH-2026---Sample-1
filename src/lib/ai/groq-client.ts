...
export function validateChatMessages(body: unknown): ChatMessage[] {
  ...
  const validated = messages.map((message): ChatMessage => {
    if (
      ...
      throw new AiServiceError("Each message needs a user/assistant role and 1–4,000 characters of text.", 400);
    }
    total += message.content.length;
    return { role: message.role, content: message.content.trim() };
  });
  if (total > 16_000 || validated[validated.length - 1].role !== "user") {
    throw new AiServiceError("Keep the conversation under 16,000 characters and end with a user question.", 400);
  }
  return validated;
}
...
export async function completeGroqChat(
  ...
  try {
    ...
    throw new AiServiceError(timeout ? "The AI request timed out. Please try again." : "Could not reach the AI service. Please try again.", timeout ? 504 : 502);
  }

  // Do not forward provider error bodies: they can contain request/config data.
  if (!response.ok) {
    if (response.status === 429) throw new AiServiceError("The AI service is busy. Please try again shortly.", 429);
    if (response.status === 401 || response.status === 403) {
      throw new AiServiceError("The AI service credentials need attention. Check GROQ_API_KEY on the server.", 503);
    }
    if (response.status === 400 || response.status === 404) {
      throw new AiServiceError("The configured AI model could not be used. Check GROQ_MODEL on the server.", 502);
    }
    throw new AiServiceError("The AI service is temporarily unavailable. Please try again.", 502);
  }
