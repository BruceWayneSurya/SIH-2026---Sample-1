...
import { googleDrivePreviewUrl, normalizeGoogleDriveUrl } from "@/lib/google-drive";

/** Google handles rendering; private/restricted files still require Drive access. */
export function GoogleDrivePreview({ url, title }: { url: string; title: string }) {
    ...
    <div className="overflow-hidden rounded-lg border border-navy-200 bg-white">
      ...
      {open && (
        <div id={viewerId} className="relative bg-slate-100">
          {loading && (
            <p role="status" className="absolute left-3 top-3 z-10 inline-flex items-center gap-2 rounded bg-white/95 px-3 py-2 text-sm text-navy-700 shadow-sm">
      ...
      <div className="space-y-1.5 border-t border-navy-100 px-3 py-2.5">
        <a href={viewUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm font-bold text-navy-700 hover:underline">
          Open PDF in Google Drive <ExternalLink className="h-3.5 w-3.5" />
          <span className="sr-only">(opens in a new tab)</span>
        </a>
        <p className="text-[12px] leading-relaxed text-slate-500">
          Preview unavailable or asking for access? Set the PDF to <b>Anyone with the link · Viewer</b> in
          Drive, or use the link above. Google controls file access and embedded previews.
        </p>
      </div>
    </div>
  );
